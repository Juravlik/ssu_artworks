...
class EngravingsTrainer:
    ...
    def _step(self, data):
        ...
        # print('ARGMAX', torch.argmax(outputs, dim=1))
        # print('LABELS', labels)
        # print('TORCH SUM', torch.sum(torch.argmax(outputs, dim=1) == labels))
        # print('LEN', len(data))

        # print('SUM(ARGMAX):', torch.sum(torch.argmax(outputs, dim=1)) / len(data))

        metric = (torch.sum(torch.argmax(outputs, dim=1) == labels) / len(labels)).detach().cpu().numpy()

        # print('METRIC', metric)

        # print('OUTPUTS: ', outputs)
        # print('OUTPUTS ARGMAX: ', torch.argmax(outputs, dim=1))
        # print('LABELS: ', labels)
        # print('METRIC: ', metric)
    ...
    def train_model(self):
        for self.epoch in tqdm(range(self.epoch, self.n_epochs + 1)):
            ...
            valid_loss, valid_metric = self._valid_step()

            self._print_step_statistic(train_loss, valid_loss, train_metric, valid_metric)
    ...
    def _print_step_statistic(self, train_loss, valid_loss, train_metric, valid_metric):
        if self.verbose:
