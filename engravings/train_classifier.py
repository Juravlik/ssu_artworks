...
from engravings.data import lock_deterministic, get_train_aug_preproc, get_valid_aug_preproc
...
parser.add_argument('--config', action="store", dest="config",
                    default=os.path.join(config_dir, "train_engravings.yaml"),
                    type=str)
...
def main(train_config: dict):
    ...
    train_dataset = object_from_dict(train_config.dataset, path=TRAIN_PATH, root=ROOT,
    ...
    valid_dataset = object_from_dict(train_config.dataset, path=VALID_PATH, root=ROOT,
    ...
    test_dataset = object_from_dict(train_config.dataset, path=TEST_PATH, root=ROOT,
                                    augmentation=get_valid_aug_preproc(PREPROCESSING_FN)
                                    )
    ...
    model = object_from_dict(model_checkpoint['config']['model'], feature_extracting=False)
    model.freeze_only_first_n_layers(num_first_layers=train_config.num_freeze_layers_on_4th_stage)
    model.load_state_dict(model_checkpoint['model'])
    model.to(DEVICE)
